...
- [2026-04-12] Entry timestamps persist in UserDefaults across app updates. If a previous build set a timestamp but didn't create a GeoLog, the new build sees alreadyTracked=true and skips GeoLog creation. Fix: add a safety net in didDetermineState — if alreadyTracked but no GeoLog exists for today (hasGeoLogEntryToday), create one anyway. This handles mid-day upgrades gracefully.
- [2026-04-13] Don't use `guard let context = modelContext else { return }` to bail silently in CLLocationManagerDelegate callbacks. iOS can wake the app in the background for a geofence event BEFORE SwiftUI's ContentView.task has run geofenceService.configure(…), so modelContext is nil — the exit GeoLog gets dropped while the departure notification still fires (NotificationService.shared needs no context). Queue events into `pendingGeoLogs` when modelContext is nil and flush them in configure(). Also never use `try? context.save()` for geofence writes — wrap in do/catch and surface via errorMessage so future save failures don't vanish.

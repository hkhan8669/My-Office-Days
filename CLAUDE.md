...
- [2026-04-07] Don't only clear stale entry timestamps from previous days in handleAppDidBecomeActive; clear ALL entry timestamps on every app foreground. Otherwise, same-day timestamps prevent new GeoLog entries when the user closes and reopens the app. The entryTimestamp dedup was designed to prevent duplicate logs, but it blocks legitimate re-detection after the user manually changes a day's status.

...
- [2026-04-10] didEnterRegion fires at actual geofence crossing = real arrival time. didDetermineState fires on app foreground = app open time. Only didEnterRegion should create GeoLog entries for accurate timestamps. didDetermineState should only log the attendance day (idempotent) and send notification on first detection of the day.
